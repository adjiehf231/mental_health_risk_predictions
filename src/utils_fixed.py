...
def plot_risk_scatter_trends(df):
    ...
    fig.update_layout(height=400, title='Risk Level vs Key Features (jittered + trend)')
    fig.write_image(os.path.join(FIGURES_DIR, 'risk_scatter_trends.png'))
    return fig

def plot_age_depression_trend(df):
    """Age vs depression score scatter with regression."""

    fig_age_dep = px.scatter(df, x='age', y='depression_score', color='mental_health_risk',
                             color_discrete_map={0:'green',1:'orange',2:'red'},
                             title="Age vs Depression Score by Risk")

    fig_age_dep.update_layout(height=400)
    fig_age_dep.write_image(os.path.join(FIGURES_DIR, 'age_depression_trend.png'))
    return fig_age_dep

def plot_age_risk_violin(df):
    """Age distribution by risk level."""
    fig_age_risk = px.violin(df, x='mental_health_risk', y='age', box=True,
                             title="Age by Mental Health Risk Level",
                             labels={'mental_health_risk': 'Risk Level'},
                             color='mental_health_risk',
                             color_discrete_map={0:'green',1:'orange',2:'red'})
    fig_age_risk.update_layout(height=400)
    fig_age_risk.write_image(os.path.join(FIGURES_DIR, 'age_by_risk.png'))
    return fig_age_risk

def plot_employment_trends(df):
    """Employment status vs risk & depression."""
    fig_emp_risk = px.histogram(df, x='employment_status', color='mental_health_risk',
                                title="Employment Status by Risk Level",
                                barmode='group',
                                color_discrete_map={0:'green',1:'orange',2:'red'})
    fig_emp_risk.update_layout(height=400)
    
    fig_emp_dep = px.box(df, x='employment_status', y='depression_score', 
                         color='employment_status',
                         title="Depression Score by Employment Status")
    fig_emp_dep.update_layout(height=400)
    
    return fig_emp_risk, fig_emp_dep
